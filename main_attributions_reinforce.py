import torch
import argparse
from src.explainability.surl import FreqRISE_Reinforce
...
def main(args):
    ...
    freqrise_file_output = f'freqrise_ds_{args.dataset}_sm_{args.use_softmax}_batchsize_{args.batch_size}_numbatches_{num_batches_freqrise}_lr_{args.lr}_alpha_{args.alpha}_beta_{args.beta}_decay_{args.decay}_numcells_{args.num_cells}_reinforce'
    if not freqrise_file_output in attributions:
        # compute FreqRISE
        print('Creating FreqRISE')
        freqrise = FreqRISE_Reinforce(model, batch_size=args.batch_size, num_batches=num_batches_freqrise, device=device, use_softmax=args.use_softmax, lr=args.lr, alpha=args.alpha, beta=args.beta, decay=args.decay, dataset=args.dataset)
...
if __name__ == '__main__':
    print("Running main_attributions_reinforce.py")
    ...
    # Baselines
    parser.add_argument('--run_baselines', type = int, default = 800, help='Window size for LRP')
