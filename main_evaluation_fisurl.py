        ...
        for key, value in attributions.items():
            ...
            if not key in evaluation['deletion curves']:
                if key == 'freqrise':
                    ...
                    filterbank = FilterBank(num_taps=args.num_taps, num_banks=args.num_banks, fs=args.fs, bandwidth=args.bandwidth)
                    fisurl_domain = 'time'
                    stft_params = firsurl_stft_params
                    cutoff = args.fisurl_cutoff
                else:
                    stft_params = lrp_stft_args
                    cutoff = None
                print(f'Computing deletion curves for {key}') ###########
                if key[:6] == 'fisurl':
                    evaluation['deletion curves'][key] = deletion_curves(model, test_loader, value, quantiles, domain=fisurl_domain, stft_params=stft_params, device=device, cutoff=cutoff, filterbank=filterbank)
                else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--fisurl_cutoff', type = eval, default = None, help='Cutoff percent for FISURL during evaluation')
    parser.add_argument('--num_banks', type=int, default=128, help='Number of banks used for FiSURL')
